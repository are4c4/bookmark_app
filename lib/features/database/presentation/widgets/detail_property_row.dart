              ...
              child: Row(
                children: [
                  SizedBox(
                    ...
                    child: Align(
                      alignment: Alignment.centerLeft,
                      child: Icon(widget.icon, size: DetailPropertyRow.propertyIconSize, color: scheme.onSurfaceVariant),
                    ),
                  ),
                  ...
                  Expanded(
                    child: Align(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(fontSize: 12.5, height: 1.0, color: scheme.onSurfaceVariant),
                      ),
                    ),
                  ),
                ],
...
class DetailSelectField<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final textColor = empty ? scheme.onSurfaceVariant.withValues(alpha: .55) : scheme.onSurface;
    ...
    return PopupMenuButton<T>(
      ...
      itemBuilder: (_) => items.entries
          .map((entry) => PopupMenuItem<T>(value: entry.key, height: 36, child: Text(entry.value, style: const TextStyle(fontSize: 12.5))))
          .toList(),
      child: SizedBox(
        height: DetailPropertyRow.rowHeight,
        child: Row(
          children: [
            Expanded(
              child: Text(
                ...
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: TextStyle(fontSize: 12.5, height: 1.0, color: textColor),
              ),
            ),
            SizedBox(
              width: DetailPropertyRow.actionColumnWidth,
              height: DetailPropertyRow.rowHeight,
              child: Center(child: Icon(Icons.keyboard_arrow_down, size: 17, color: scheme.onSurfaceVariant)),
            ),
          ],
        ),
      ),
