...
void main() {
  test('Stage1 Photo filter delegates to canonical Bookmark backlink stream', () {
    final stage1 =
        File('lib/views/bookmark_unified_stage1_page.dart').readAsStringSync();
    final query = File('lib/views/bookmark_query_engine.dart').readAsStringSync();

    expect(
      stage1,
      contains('stream: _watchBookmarksForActiveFilters(),'),
    );
    expect(
      stage1,
      contains('widget.repository.watchBookmarksForPhoto(photo)'),
    );
