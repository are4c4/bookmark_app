...
void main() {
  test('Image definition reuses system image type and installs defaults', () async {
    ...
    expect(
      first.fileProperty.config['system'],
      isTrue,
    );
    expect(
      first.fileProperty.config['hidden'],
      isTrue,
    );
    ...
    expect(
      first.storageOwnershipProperty.config['system'],
      isTrue,
    );
    expect(
      first.storageOwnershipProperty.config['hidden'],
      isTrue,
    );
    ...
    expect(defaults?.propertyOrder, <int>[
      ...
      first.contentTypeProperty.id,
      first.sourceUrlProperty.id,
      first.storageOwnershipProperty.id,
  ...
  test(
    'Image definition upgrades existing internal storage fields to hidden',
    () async {
      final database = AppDatabase.forTesting(NativeDatabase.memory());
      addTearDown(database.close);
      final workspaceId = await WorkspaceStore(database).initialize();
