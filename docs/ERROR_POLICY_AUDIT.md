...
| `lib/views/global_search_page.dart` | search/index failure uses a stable retry-oriented boundary | user-visible failure | #331 replaced raw exception rendering with a boolean failure state, fixed debug diagnostics + stack trace, and retry coverage. |
| `lib/views/settings_page.dart` | backup/settings operation catches use stable retry-oriented messages | user-visible failure | #332/#333 and follow-up slices avoid raw implementation exception text while preserving existing operations and retry/confirmation behavior. |
| `lib/main.dart` | initialization/Profile-switch failure remains fail-closed; raw exception was previously interpolated into the fatal screen | user-visible/fail-closed initialization | Current Refactor slice keeps startup/profile-switch failure semantics unchanged, replaces raw exception rendering with a stable retry-oriented message, and emits debug/assert-only fixed operation diagnostics + stack traces without exception text or profile/path content. |
