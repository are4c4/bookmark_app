...
void main() {
  test('detail Relation context resolves outgoing and backlink neighbors', () async {
    ...
    ObjectDetailRelationContextLoader loader() =>
        ObjectDetailRelationContextLoader(
    ...
    final person =
        await loader().load(objectTypeId: personTypeId, objectId: personId);

    expect(book?.outgoing.single.property.id, authorId);
    expect(book?.outgoing.single.targetObject.title, 'Alice');
    expect(book?.backlinks, isEmpty);
    expect(person?.backlinks.single.property.id, authorId);
