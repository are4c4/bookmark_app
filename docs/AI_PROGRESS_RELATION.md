...
Primary Tag-hierarchy correctness slice.

Required:
- canonical Tag -> Parent(Tag) relation with cardinality 0..1;
- reject self-parenting and indirect cycles before partial mutation;
- reject non-Tag parent targets and malformed/ambiguous state;
- group membership references a valid TagGroup without a second hierarchy store;
- direct Object -> Tag assignments remain independent of derived ancestors;
- delete/detach/backlink/index behavior stays on canonical Relation infrastructure.

No descendant-filter UI/query implementation here; that belongs to C/#1053.
...
## Resume sequence
1. re-read live #1052/#1042/#1045 and current PR ownership;
2. choose one focused Issue and one active branch/PR;
3. implement the smallest integrity kernel below presentation;
