...
void main() {
  test('legacy-backed Person detail edits keep canonical and legacy state equal', () async {
    ...
    expect((await database.select(database.people).get()).single.name, 'After');
    ...
    expect((await database.select(database.people).get()).single.note, 'new note');
    ...
    expect((await database.select(database.people).get()).single.note, isNull);
  });
  ...
  test('native canonical Person remains editable without creating legacy projection', () async {
    ...
    expect(
      await bridge.legacyPersonIdForObject(workspaceId, objectId),
      isNull,
    );
  });

  test('missing Person mapping fails closed before canonical mutation', () async {
    final database = AppDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    final workspaceId = await WorkspaceStore(database).initialize();
    final genericStore = GenericDatabaseStore(database);
  ...
  test('ambiguous legacy identity fails closed before Person detail mutation', () async {
    final database = AppDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    final workspaceId = await WorkspaceStore(database).initialize();
    final genericStore = GenericDatabaseStore(database);
