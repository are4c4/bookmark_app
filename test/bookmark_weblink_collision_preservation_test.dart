...
void main() {
  test('Bookmark reconciliation preserves legacy URL on canonical Weblink collision', () async {
    ...
    final bookmarkId =
        (await database
                .customSelect("SELECT id FROM bookmarks WHERE title = 'Legacy'")
                .getSingle())
            .read<int>('id');
    await database.customStatement(
    ...
    final bookmarkObject = (await sync.objectStore.listObjects(bookmarkType.id))
        .single;
    expect(
    ...
    final preservedLegacyUrl = await database
        .customSelect(
