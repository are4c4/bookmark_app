...
void main() {
  test('default support bundle contains technical provenance without user content', () async {
  ...
  test('unsafe provider field fails closed without hiding core diagnostics', () async {
    ...
    final decoded = jsonDecode((await service.collect()).prettyJson) as Map<String, dynamic>;

    expect(decoded['database']['schemaVersion']['value'], 16);
    expect(decoded['providers']['unsafe_provider']['status'], 'unavailable');
    expect(decoded['providers']['unsafe_provider']['code'], 'privacy_rejected');
    expect(jsonEncode(decoded), isNot(contains('/Users/private/Vault')));
    expect(events.snapshot().single.code, 'privacy_rejected');
  });

  test('one broken provider stays isolated and does not export exception text', () async {
    final service = SupportDiagnosticsService(
      buildProvenance: _provenance,
      databaseSchemaVersion: 16,
    ...
    expect(decoded['providers']['object_health']['fields']['available']['value'], isTrue);
  ...
  test('snapshot output is deterministic with fixed inputs and provider ordering', () async {
    SupportDiagnosticsService service(Iterable<SupportDiagnosticProvider> providers) =>
