...
- #489/#495 — Lane D primitive File/Image work using the Storage-owned managed-file contract. The filesystem dependency is delivered on `main`; take new Lane F work only for a concrete filesystem/Vault gap.
...
Lane F provides the complete filesystem seam requested by Lane D:
...
- Receipt rollback uses the same ownership-gated delete boundary.

Lane D has already started consuming this contract on `main`: commit `eb2f0e6d1ca06feefb2664e20407e21622d5f0a4` composes `VaultManagedFileCopyService.copyIntoVault()` with canonical generic File Object creation and rolls back the Storage receipt if Object creation fails. File identity, MIME/content routing and domain mutation remain Lane D responsibilities. Persistent physical deletion may use #771 only when Lane D has persisted/retained the explicit Storage ownership key; Lane F must not infer ownership from path location.
...
- Lane D generic File import composition at `eb2f0e6d1ca06feefb2664e20407e21622d5f0a4` verifies source preservation, canonical File metadata, Vault-relative managed path, and rollback of the exact new copy when Object creation fails.
...
- Lane D is no longer blocked on the managed-copy filesystem seam and already uses it for canonical generic File import. Ownership-gated persistent delete remains available when Lane D has a concrete lifecycle requiring it.
...
4. For #489/#495, only take new Lane F work if Lane D reports a concrete filesystem/Vault contract gap. Do not implement primitive identity/MIME routing in Lane F.

## Work in progress
No production-code Storage PR is currently required after #771. The managed-file dependency is delivered and already consumed by Lane D; Vault lifecycle code is complete. This handoff update is the current Lane F bookkeeping slice.
