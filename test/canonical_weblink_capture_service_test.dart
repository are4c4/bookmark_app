...
void main() {
  test(
    'direct capture reuses normalized URL without Bookmark authority',
    () async {
  ...
  test(
    'canonical URL collision fails closed without creating another target',
    () async {
      final database = AppDatabase.forTesting(NativeDatabase.memory());
      addTearDown(database.close);
      final workspaceId = await WorkspaceStore(database).initialize();
  ...
  test(
    'first concurrent equivalent captures create one definition and target',
    () async {
      final database = AppDatabase.forTesting(NativeDatabase.memory());
      addTearDown(database.close);
      final workspaceId = await WorkspaceStore(database).initialize();
      ...
      final captured = await Future.wait([
        ...
        CanonicalWeblinkCaptureService(
          weblinks: harness.weblinks,
        ).capture(workspaceId: workspaceId, url: 'https://example.com/article'),
      ]);
      final definition = await harness.weblinks.ensureDefinition(workspaceId);
      ...
      expect(
        await harness.objectStore.listObjectTypes(workspaceId),
        hasLength(1),
      );
    },
  );
  ...
  test(
    'capture reuses persisted canonical identity after database restart',
    () async {
      ...
      try {
        ...
        final restartedWorkspaceId = await WorkspaceStore(database)
            .initialize();
        ...
        final definition = await restartedHarness.weblinks.ensureDefinition(
          restartedWorkspaceId,
        );

        expect(restartedWorkspaceId, workspaceId);
        expect(second.id, firstId);
        expect(
          await restartedHarness.objectStore.listObjects(
  ...
  test(
    'invalid direct capture fails before creating a Weblink ObjectType',
    () async {
      final database = AppDatabase.forTesting(NativeDatabase.memory());
      addTearDown(database.close);
      final workspaceId = await WorkspaceStore(database).initialize();
