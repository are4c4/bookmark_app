...
void main() {
  test(
    'generic Person delete removes legacy projection, detaches backlinks, and preserves profile Image',
    () async {
      ...
      expect(
        ObjectRelationValue.fromJson(
          survivingBook.values[authorPropertyId],
        ).objectIds,
        isEmpty,
      );
  ...
  test('generic delete permits a native canonical Person without legacy projection',
      () async {
    final database = AppDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    final workspaceId = await WorkspaceStore(database).initialize();
