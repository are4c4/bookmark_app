...
void main() {
  Future<void> expectSavedStateConflict({
    ...
    final preservedLegacy = await database.customSelect(
  ...
  test('Favorite conflict fails before Bookmark Relation or URL retirement',
      () async {
    await expectSavedStateConflict(favorite: true);
  });

  test('Reading Status conflict fails before Bookmark Relation or URL retirement',
      () async {
    await expectSavedStateConflict(readingStatus: 'done');
  });

  test('Storage State conflict fails before Bookmark Relation or URL retirement',
      () async {
    await expectSavedStateConflict(storageState: 'archived');
  });

  test('Genre conflict fails before Bookmark Relation or URL retirement',
      () async {
    await expectSavedStateConflict(genre: 'Research');
  });

  test('Rating conflict fails before Bookmark Relation or URL retirement',
      () async {
    await expectSavedStateConflict(rating: 5);
  });

  test('preserved conflict can reconcile after restart without duplicate Weblink',
      () async {
    final database = AppDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    final workspaceId = await WorkspaceStore(database).initialize();
    ...
    final firstWeblinkType =
        (await firstSync.systemObjectStore.getSystemObjectType(
      workspaceId: workspaceId,
      systemKey: WeblinkObjectService.systemKey,
    ))!;
    expect(
    ...
    final report =
        await restarted.bookmarkWeblinkBridge.syncWorkspace(workspaceId);
    ...
    final bookmarkType = (await restarted.systemObjectStore.getSystemObjectType(
      workspaceId: workspaceId,
      systemKey: CoreObjectBridge.bookmarkSystemKey,
    ))!;
    final relation = bookmarkType.properties.singleWhere(
    ...
    final bookmarks = await restarted.objectStore.listObjects(bookmarkType.id);
    expect(bookmarks, hasLength(2));
    for (final bookmark in bookmarks) {
      expect(
        ObjectRelationValue.fromJson(bookmark.values[relation.id]).objectIds,
      ...
      final edges = await restarted.objectStore.outgoingRelations(bookmark.id);
      final weblinkEdges =
          edges.where((edge) => edge.propertyId == relation.id).toList();
