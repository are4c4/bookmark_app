...
void main() {
  test('Tag management move and undo keep canonical hierarchy and compatibility state equivalent', () async {
    ...
    final movedChild = (await repository.watchTags().first).singleWhere(
      (tag) => tag.id == childId,
    );
    ...
    expect(movedHierarchy.parentByTagObjectId[childObjectId!], parentObjectId);
    ...
    final restoredChild = (await repository.watchTags().first).singleWhere(
      (tag) => tag.id == childId,
    );
