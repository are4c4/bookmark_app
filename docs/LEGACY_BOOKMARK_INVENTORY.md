...
| `lib/widgets/person_role_properties.dart` | canonical product behavior / compatibility | Shared Property add flow is integrated (#350); Person assignment remains on existing repository/canonical semantics. |
| `lib/widgets/bookmark_reorderable_properties.dart` | canonical product behavior / compatibility | Remaining person-role Property-add convergence landed in #366 and now uses the shared `PropertyAddPopover`. The host remains live Bookmark behavior; do not treat UI convergence as retirement. |
...
GlobalSearch, Settings, attachment import and bootstrap/Profile-switch raw-error boundaries are already stabilized (#331–#335/#364/#368). Re-audit before assuming an old raw-error candidate still exists. Prefer a small independently testable host; do not overlap active Object-owned shared hotspots.
