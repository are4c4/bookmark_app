...
void main() {
  ...
  test('created Bookmark Image save rejects existing Relation index drift before sync',
      () async {
    final database = AppDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    final workspaceId = await WorkspaceStore(database).initialize();
    ...
    final definition = await images.ensureDefinition(workspaceId);
    final existingImageId = await objectStore.createObject(
      objectTypeId: definition.objectType.id,
      title: 'Existing Image',
    );
    final newImageId = await objectStore.createObject(
      objectTypeId: definition.objectType.id,
      title: 'New Image',
    );

    await database.customStatement(
      "INSERT INTO bookmarks(url, title) VALUES ('https://canonical-drift.example', 'Drift Bookmark')",
    );
    final bookmarkId = (await database.customSelect(
      "SELECT id FROM bookmarks WHERE url = 'https://canonical-drift.example'",
    ).getSingle())
        .read<int>('id');
    await database.customStatement(
      'INSERT INTO bookmark_workspace(bookmark_id, workspace_id) VALUES (?, ?)',
    ...
    await service.saveImages(
      state: state,
      selectedObjectIds: <int>[existingImageId],
    );
    state = (await service.load(
      workspaceId: workspaceId,
      bookmarkId: bookmarkId,
    ...
    await database.customStatement(
      ...
      <Object>[state.bookmarkObjectId, state.images.property.id],
    );
    ...
    expect(after.images.selectedObjectIds, <int>[existingImageId]);
    final position = (await database.customSelect(
      'SELECT position FROM object_relation_edges '
      'WHERE source_object_id = ${after.bookmarkObjectId} '
      'AND property_id = ${after.images.property.id}',
    ).getSingle())
        .read<int>('position');
    expect(position, 7);
    final legacyBookmarkPhotoCount = (await database.customSelect(
