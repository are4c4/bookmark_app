...
A fresh metadata audit found one concrete follow-up: `ImageObjectService.findOrCreateManaged(...)` still persists incoming `contentType` directly under a missing-only contract, unlike the normalized File/Weblink boundaries. A malformed or parameterized first value can therefore become durable Image metadata. The next focused Image slice should apply the same basic trim/lowercase/parameter-strip/MIME-shape normalization before persistence, without restricting the value to `image/*` and without changing Image routing or identity.
...
#709 and #711 harden durable metadata only; neither changes routing target selection or content-first classification precedence.
...
Local Flutter/Dart execution is unavailable in the connector-only runtime. No Lane D production PR remains open at this checkpoint; refresh from latest main before the next primitive change and require the normal Flutter CI on its final head before integration.

## Hotspot / concurrency state
No shared hotspot lease is held by Lane D at this checkpoint. The latest open-PR audit found no competing owner for `ImageObjectService`, but shared presentation hosts remain active cross-lane territory.
...
Lane F #691 remains storage-migration-only and still does not provide the generic managed-copy/ownership seam. Lane C #696/#703 and active Object/Relation/Refactor work do not own primitive data services. Avoid `generic_database_page.dart`, `app_shell.dart`, `object_inspector_page.dart`, `people_management_page.dart`, `photo_management_page.dart`, Relation internals and Vault lifecycle code unless a fresh ownership audit proves a patch is safe.

## Exact next actions
1. Implement the focused `ImageObjectService` content-type normalization regression identified above: malformed MIME must remain missing, valid parameterized MIME must normalize, and reimport must preserve existing non-empty metadata/identity.
2. After that slice, re-audit primitive metadata/native capability consumers only for another concrete fail-open or missing-only enrichment defect; do not add speculative abstractions.
