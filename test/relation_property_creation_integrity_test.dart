...
void main() {
  test('relation Property creation rejects a missing target ObjectType', () async {
    ...
    await expectLater(store.createRelationProperty(objectTypeId: bookTypeId, name: 'Author', targetObjectTypeId: 999999), throwsArgumentError);
  ...
  test('relation Property creation rejects cross-workspace targets', () async {
    ...
    final bookTypeId = await store.createObjectType(workspaceId: workspaceA, name: 'Book');
    final personTypeId = await store.createObjectType(workspaceId: workspaceB, name: 'Person');
    await expectLater(store.createRelationProperty(objectTypeId: bookTypeId, name: 'Author', targetObjectTypeId: personTypeId), throwsArgumentError);
    expect((await store.getObjectType(bookTypeId))!.properties, isEmpty);
  });
}
