  ...
  group('historical database fixtures', () {
    test('legacy v8 migrates losslessly and reopening is idempotent', () async {
      ...
      expect(
        ...
        firstSnapshot,
        reason:
            'legacy-v8 logical state changed after an already-migrated reopen',
      );
      await reopened.close();
    });

    test(
      'object-era v16 preserves Object Body Relation and View state',
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'bookmark-v16-fixture-',
        );
        addTearDown(() => directory.delete(recursive: true));
        final file = File('${directory.path}/database.sqlite');
        ...
        _expectObjectEra(
          firstSnapshot,
          checkpoint: 'object-era-v16 first open',
        );
        await first.close();
...
void _expectLegacyV8(
  ...
  expect(
    snapshot['version'],
    16,
    reason: '$checkpoint did not reach current schema',
  );
  final bookmark = snapshot['bookmark']! as Map<String, Object?>;
  expect(
    bookmark['url'],
    'https://fixture.invalid/article',
    reason: '$checkpoint lost Bookmark URL',
  );
  expect(
    bookmark['title'],
    'Synthetic legacy bookmark',
    reason: '$checkpoint lost Bookmark title',
  );
  expect(
    bookmark['thumbnail'],
    'managed/thumbnails/synthetic.jpg',
    reason: '$checkpoint lost media reference',
  );
  expect(
    bookmark['createdAt'],
    1704067200,
    reason: '$checkpoint changed creation time',
  );
  expect(
    bookmark['readingStatus'],
    'unread',
    reason: '$checkpoint changed reading lifecycle',
  );
  expect(
    bookmark['storageState'],
    'active',
    reason: '$checkpoint changed storage lifecycle',
  );
  ...
  expect(
    person['name'],
    'Synthetic Person',
    reason: '$checkpoint lost Person identity',
  );
  expect(
    person['role'],
    '出演者',
    reason: '$checkpoint lost/mis-migrated Bookmark-Person relation',
  );

  final photo = snapshot['photo']! as Map<String, Object?>;
  expect(
    photo['path'],
    'managed/photos/synthetic-cover.jpg',
    reason: '$checkpoint lost Photo path',
  );
  expect(
    photo['isCover'],
    1,
    reason: '$checkpoint lost Bookmark-Photo cover relation',
  );

  final view = snapshot['view']! as Map<String, Object?>;
  expect(
    view['name'],
    'Synthetic legacy view',
    reason: '$checkpoint lost Saved View',
  );
  expect(
    view['layout'],
    'table',
    reason: '$checkpoint changed Saved View layout',
  );
  expect(
    view['query'],
    'fixture needle',
    reason: '$checkpoint changed Saved View query',
  );
  expect(
    view['visibleProperties'],
    'image,url,tags',
    reason: '$checkpoint changed Saved View properties',
  );
}
...
Future<Map<String, Object?>> _objectEraSnapshot(AppDatabase database) async {
  ...
  final project = (await objectStore.listObjects(21))
      .singleWhere((object) => object.id == 100);
  ...
  return <String, Object?>{
    ...
    'objectType': <String, Object?>{
      ...
      'relationProperty': projectType?.properties
          .singleWhere((property) => property.id == 30)
          .name,
      'relationTargetType': projectType?.properties
          .singleWhere((property) => property.id == 30)
          .targetObjectTypeId,
    },
...
void _expectObjectEra(
  ...
  expect(
    type['name'],
    'Project',
    reason: '$checkpoint lost ObjectType identity',
  );
  expect(
    type['relationProperty'],
    'Members',
    reason: '$checkpoint lost Relation property',
  );
  expect(
    type['relationTargetType'],
    20,
    reason: '$checkpoint changed Relation target type',
  );
  ...
  expect(
    object['title'],
    'Synthetic Project',
    reason: '$checkpoint lost Object identity',
  );
  expect(object['members'], <int>[
    101,
    102,
  ], reason: '$checkpoint changed Relation serialized order');

  final body = snapshot['body']! as List<Map<String, Object?>>;
  expect(body.map((block) => block['id']).toList(), <String>[
    'p-1',
    'p-2',
  ], reason: '$checkpoint changed Body order');
  expect(body.map((block) => block['text']).toList(), <String>[
    'First fixture block',
    'Second fixture block',
  ], reason: '$checkpoint lost Body content');

  final edges = snapshot['edges']! as List<Map<String, Object?>>;
  expect(edges.map((edge) => edge['target']).toList(), <int>[
    101,
    102,
  ], reason: '$checkpoint changed Relation edge targets/order');
  expect(edges.map((edge) => edge['position']).toList(), <int>[
    0,
    1,
  ], reason: '$checkpoint changed Relation edge positions');

  final view = snapshot['view']! as Map<String, Object?>;
  expect(
    view['databaseKey'],
    'custom:21',
    reason: '$checkpoint detached Database View',
  );
  expect(
    view['layout'],
    'table',
    reason: '$checkpoint changed Database View layout',
  );
  expect(
    view['visibleProperties'],
    'title,Members',
    reason: '$checkpoint changed View properties',
  );
}
