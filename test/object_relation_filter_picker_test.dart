...
void main() {
  testWidgets(
    'shows canonical titles, preserves ids and exposes missing targets',
    (tester) async {
      ...
      expect(
        find.descendant(
          of: find.byKey(const ValueKey('relation-filter-selected-2')),
      ...
      await tester.pumpAndSettle();
      await tester.tap(
        find.byKey(const ValueKey('relation-filter-candidate-1')),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'keyboard arrows, Enter and Escape operate on search candidates',
    (tester) async {
      var selected = <int>[];
      late StateSetter setHostState;
      final candidates = <ObjectIdentitySearchResult>[
