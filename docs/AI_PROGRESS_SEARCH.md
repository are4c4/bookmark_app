...
- the same CJK fallback may compare deterministic Japanese width-compatibility variants for the half-width punctuation/Katakana block, including composed dakuten/handakuten forms such as `ｶﾞ` ↔ `ガ`; internal punctuation such as `東・京` ↔ `東･京` is preserved rather than erased, and this is intentionally narrower than general NFKC/transliteration/fuzzy search so ordinary Latin infix semantics remain unchanged;
- Japanese dakuten/handakuten comparison canonicalizes both each CJK query needle and candidate text from the same Object FTS projection to one deterministic full-width/composed Japanese comparison form before substring matching. This covers precomposed, decomposed and partially mixed forms such as `ガイド`, `カ`+U+3099+`イド`, and per-character mixtures without enumerating every composition combination or persisting a normalized copy; hiragana↔katakana folding, transliteration and unrelated compatibility normalization remain out of scope;
- CJK fallback candidates are read in deterministic rank/Object-id order in bounded pages; mixed Latin+CJK queries keep non-CJK terms on FTS prefix MATCH/bm25, while pure fallback ordering stays deterministic;
