...
void main() {
  ...
  test('legacy resync preserves canonical TagGroup state', () async {
    ...
    final groups = await database.customSelect(
      'SELECT id, name FROM tag_groups ORDER BY sort_order',
    ).get();
  ...
  test('legacy resync preserves an explicitly cleared canonical Parent', () async {
    final database = AppDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    final workspaceId = await WorkspaceStore(database).initialize();
    final objectStore = ObjectStore(GenericDatabaseStore(database));
