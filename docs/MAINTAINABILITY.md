...
Canonical feature presentation also currently has **7 files** that import `app_database.dart` directly. Those existing dependencies are not being removed by fiat: several widgets still receive `AppDatabase` as part of their current contract. The new import metric therefore starts at the observed baseline of **7** and prevents new direct database imports from being added while those existing dependencies are classified and migrated behind meaningful application-facing boundaries. Do not introduce a wrapper solely to lower this count.
